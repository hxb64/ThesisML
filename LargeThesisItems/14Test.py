...
small_model = lgb.LGBMRegressor(n_estimators=int(best_num_estimators/1.18))
...
num_noise_vectors = 40
for i in range(num_noise_vectors):
    x_valid_noisy = x_valid_normalized + np.random.normal(0, 0.025, x_valid_normalized.shape)  # Adding Gaussian noise
